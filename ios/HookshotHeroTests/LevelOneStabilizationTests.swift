...
final class LevelOneStabilizationTests: XCTestCase {
    ...
    func testLethalInteractionSynchronizesAndStopsSameFrameRewards() throws {
        ...
        simulation.onStatusChange = { statuses.append(($0.health, $0.score)) }; simulation.onOutcome = { outcomes.append($0) }
    ...
    func testDeterministicLevelOneMapPlaythroughReachesVictory() throws {
        ...
        XCTAssertEqual(simulation.outcome, .won); XCTAssertEqual(session.state, .won); XCTAssertEqual(session.score, 220); XCTAssertEqual(session.health, 4)
        XCTAssertEqual(simulation.feedbackEvents.filter { if case .levelCompleted = $0.kind { true } else { false } }.count, 1)
        XCTAssertNil(router.activeSession); XCTAssertEqual(router.path.count, 1)
        guard case .results(let result) = router.path[0] else { return XCTFail("Expected one results route") }
        XCTAssertEqual(result.score, 220); XCTAssertEqual(result.levelID, .init(rawValue: "level-1")); XCTAssertEqual(result.outcome, .won)
        XCTAssertEqual(progression.progression.highScore, 220); XCTAssertEqual(progression.progression.completedLevelIDs, [.init(rawValue: "level-1")])
        router.finishGame(sessionID: session.identifier, outcome: .won); XCTAssertEqual(router.path, [.results(result)])
    }

    func testLethalFirstQueuedCommandAtomicallyStopsMovementAndHook() throws {
        let lethal = (0..<3).map { _ in WorldEntity(id: EntityID(), kind: .mine, position: .init(row: 49, column: 27)) }
        let simulation = try LevelOneSimulation(entities: lethal)
        var outcomes: [GameOutcome] = []
        simulation.onOutcome = { outcomes.append($0) }
        simulation.input.send(.move(.up)); simulation.input.send(.move(.left)); simulation.input.send(.fireHook)
        simulation.update(deltaTime: 0.01)
        XCTAssertEqual(simulation.outcome, .lost); XCTAssertEqual(simulation.player.health, 0)
        XCTAssertEqual(simulation.player.position, .init(row: 49, column: 27)); XCTAssertEqual(simulation.player.facing, .up)
        XCTAssertEqual(simulation.player.hookshot.phase, .idle); XCTAssertEqual(outcomes, [.lost])
        let snapshot = simulation.player; let chest = simulation.chestOpen; let feedback = simulation.feedbackEvents
        simulation.attemptMove(.right); simulation.fireHook(); simulation.activateChestAndExit()
        for _ in 0..<5 { simulation.update(deltaTime: 1) }
        XCTAssertEqual(simulation.player, snapshot); XCTAssertEqual(simulation.chestOpen, chest)
        XCTAssertEqual(simulation.feedbackEvents, feedback); XCTAssertEqual(outcomes, [.lost])
    }

    func testStatusPublishesOnlyCoherentChanges() throws {
        let coin = WorldEntity(id: EntityID(), kind: .coin, position: .init(row: 49, column: 27))
        let simulation = try LevelOneSimulation(entities: [coin]); var statuses: [PlayerStatusSnapshot] = []
        simulation.onStatusChange = { statuses.append($0) }
        simulation.update(deltaTime: 0.01); XCTAssertTrue(statuses.isEmpty)
        simulation.input.send(.move(.up)); simulation.update(deltaTime: 0.01)
        XCTAssertEqual(statuses, [.init(health: 3, score: 10)])
        simulation.update(deltaTime: 0.01); XCTAssertEqual(statuses.count, 1)
    }

    func testDirectionPressStateMachineSeparatesTapHoldAndCancellation() {
        var press = DirectionPressController()
        XCTAssertEqual(press.press(.up), []); XCTAssertEqual(press.release(.up), [.moveOnce(.up)]); XCTAssertEqual(press.state, .idle)
        XCTAssertEqual(press.release(.up), [], "a quick tap cannot dispatch twice")
        _ = press.press(.left); XCTAssertEqual(press.holdThreshold(.left), [.beginHold(.left)])
        XCTAssertEqual(press.release(.left), [.endHold(.left)]); XCTAssertEqual(press.state, .idle)
        _ = press.press(.right); XCTAssertEqual(press.cancel(.right), []); XCTAssertEqual(press.cancel(.right), [])
        _ = press.press(.down); _ = press.holdThreshold(.down); XCTAssertEqual(press.cancel(.down), [.endHold(.down)])
    }

    func testProductionSeed496913SpawnsSafeDeterministicFullFootprints() throws {
        let level = LevelOneDefinition.make(); var firstRNG = SeededRandomNumberGenerator(seed: 496_913); var secondRNG = SeededRandomNumberGenerator(seed: 496_913)
        let first = try SpawnService.spawn(in: level, using: &firstRNG); let second = try SpawnService.spawn(in: level, using: &secondRNG)
        XCTAssertEqual(first.map(\.kind), second.map(\.kind)); XCTAssertEqual(first.map(\.position), second.map(\.position))
        XCTAssertEqual(first.count, 15); XCTAssertEqual(first.filter{$0.kind == .mine}.count, 3); XCTAssertEqual(first.filter{$0.kind == .cabbage}.count, 2); XCTAssertEqual(first.filter{$0.kind == .coin}.count, 10)
        ...
        for (index, entity) in first.enumerated() { let region = CollisionProfile.footprint(for: entity.kind).region(at: entity.position); XCTAssertFalse(level.isBlocked(region)); XCTAssertFalse(level.overlapsLava(region)); XCTAssertFalse(protected.contains{$0.intersects(region)}); for other in first.dropFirst(index + 1){XCTAssertFalse(region.intersects(CollisionProfile.footprint(for:other.kind).region(at:other.position)))}}
    }

    func testMineEffectDescriptorRespectsReducedMotion() {
        let standard = MineDestructionEffectDescriptor.make(reducedMotion: false, duration: 2.4), reduced = MineDestructionEffectDescriptor.make(reducedMotion: true, duration: 2.4)
        XCTAssertEqual(standard.style, .standard); XCTAssertTrue(standard.scales); XCTAssertTrue(standard.isVisible)
        XCTAssertEqual(reduced.style, .reducedMotion); XCTAssertFalse(reduced.scales); XCTAssertTrue(reduced.isVisible)
        XCTAssertEqual(standard.duration, reduced.duration); XCTAssertGreaterThan(reduced.duration, 0)
    }
