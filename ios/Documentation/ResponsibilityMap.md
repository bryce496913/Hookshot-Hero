...
| `GameEngine` | Swing frame/panel, global keyboard dispatcher, Timer update/repaint | UI/simulation coupled; swallowed sleep error; listener lifetime risk | SwiftUI lifecycle + `GameScene` | Foundation |
...
| `GameOptions` | Mutable mode/music/volume/display options | No durable preference repository identified | `GameSettings` + session configuration | Foundation |
