...
struct GameplayView: View {
    ...
    init(session: GameSession, returnToMenu: @escaping () -> Void) { self.session=session;self.returnToMenu=returnToMenu;_scene=State(initialValue:GameScene(session:session,runtime:session.runtime,generation:session.runtimeGeneration)) }
    var body: some View {
        VStack(spacing:8) { hud
            ZStack { SpriteView(scene:scene).id(scene.runtimeGeneration).aspectRatio(1,contentMode:.fit).background(.black).accessibilityHidden(true).accessibilityIdentifier("gameBoard"); GameplayFeedbackOverlay(feedback: session.uiSnapshot.feedback, reducedMotion: session.configuration.reducedMotion) }
