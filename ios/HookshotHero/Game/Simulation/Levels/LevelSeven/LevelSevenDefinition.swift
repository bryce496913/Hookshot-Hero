...
enum LevelSevenDefinition {
  static let lavaAnchors: [GridPosition] = {
    ...
    var javaDerivedPositions: [GridPosition] = []
    javaDerivedPositions.append(
      contentsOf: [8].flatMap { row in
        [12, 16, 20].map { GridPosition(row: row, column: $0) }
      })
    javaDerivedPositions.append(
      contentsOf: [16].flatMap { row in
        [12, 16, 20].map { GridPosition(row: row, column: $0) }
      })
    javaDerivedPositions.append(
      contentsOf: [36].flatMap { row in
        [12, 16, 20].map { GridPosition(row: row, column: $0) }
      })
    javaDerivedPositions.append(
      contentsOf: [40].flatMap { row in
        [40, 44].map { GridPosition(row: row, column: $0) }
      })
    javaDerivedPositions.append(
      contentsOf: [52].flatMap { row in
        [32, 36, 40].map { GridPosition(row: row, column: $0) }
      })
    javaDerivedPositions.append(
      contentsOf: [16].flatMap { row in
    ...
    javaDerivedPositions.append(
      contentsOf: [28, 32].map {
        GridPosition(row: $0, column: 48)
      })
    javaDerivedPositions.append(
      contentsOf: [48, 52].map {
        GridPosition(row: $0, column: 8)
      })
    javaDerivedPositions.append(
      contentsOf: [8, 12].flatMap { row in
        [32, 36, 40, 44, 48, 52].map { GridPosition(row: row, column: $0) }
      })
    javaDerivedPositions.append(
      contentsOf: [20, 24, 28].flatMap { row in
        [4, 8].map { GridPosition(row: row, column: $0) }
      })
    javaDerivedPositions.append(
      contentsOf: [32, 36, 40].flatMap { row in
        [40, 44].map { GridPosition(row: row, column: $0) }
      })
    javaDerivedPositions.append(contentsOf: [
  ...
  static let wallAnchors: [GridPosition] = {
    var anchors: [GridPosition] = []
    anchors.append(
      contentsOf: [20, 24, 28, 32].flatMap { row in
        [12, 16, 20].map { GridPosition(row: row, column: $0) }
      })
    anchors.append(
      contentsOf: [48, 52].flatMap { row in
        [48, 52].map { GridPosition(row: row, column: $0) }
      })
    anchors.append(
      contentsOf: [40, 44, 48].flatMap { row in
        [20, 24, 28, 32].map { GridPosition(row: row, column: $0) }
      })
    anchors.append(
      contentsOf: [12, 16, 20, 24].map {
        GridPosition(row: 4, column: $0)
      })
    anchors.append(
      contentsOf: [4, 8, 12, 16, 20].map {
        GridPosition(row: 12, column: $0)
      })
    anchors.append(
      contentsOf: [28, 32, 36, 40, 44].map {
        GridPosition(row: 16, column: $0)
      })
    anchors.append(
      contentsOf: [36, 40, 44, 48, 52].map {
        GridPosition(row: 24, column: $0)
      })
    anchors.append(
      contentsOf: [4, 8, 12, 16].map {
        GridPosition(row: 40, column: $0)
      })
    anchors.append(contentsOf: [36, 40].map { GridPosition(row: 48, column: $0) })
    anchors.append(contentsOf: [48, 52].map { GridPosition(row: 44, column: $0) })
    anchors.append(
      contentsOf: [24, 28, 32, 36].map {
        GridPosition(row: 32, column: $0)
      })
    anchors.append(contentsOf: [48, 52].map { GridPosition(row: $0, column: 12) })
    anchors.append(contentsOf: [20, 24].map { GridPosition(row: $0, column: 28) })
