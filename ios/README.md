...
Lidia starts on the original 60×60 board at row 50, column 27. Native touch controls provide four-directional, grid-based joystick movement and tap-or-drag Grapple aiming with the original 19-cell range. Walls, the six-cell top exit, the closed lower door, lava damage and safe-position restoration all participate in collision. Coins, cabbages, mines, and the talking chest are interactive. Victory, defeat, automatic results navigation, high scores, and Level 1 completion persistence are implemented.
...
* **Movement:** the visible movement control is an anchored virtual joystick. Movement remains four-directional and the simulation remains grid based; joystick input snaps to the dominant cardinal axis rather than producing analog world-space movement. A center dead zone prevents accidental movement. Holding the joystick continues movement, while returning it to center or releasing it stops movement. Held movement is also cancelled on pause, dialogue, level transition, backgrounding, and view disappearance.
* **Grapple:** the circular Grapple control supports both quick firing and explicit aiming. Tap it to fire in Lidia's current facing direction, or drag from it to select a cardinal aim direction and release to fire in that direction. Directional Grapple aiming does not move Lidia. Grapple range remains 19 cells.
* **Pause:** suspends play and offers Resume or Return to Menu.
* **Accessible alternatives:** VoiceOver and Switch Control users do not need to operate the joystick or perform drag gestures. The app exposes the semantic actions **Move Up**, **Move Down**, **Move Left**, **Move Right**, and **Grapple**, plus **Grapple Up**, **Grapple Down**, **Grapple Left**, and **Grapple Right** directional actions. These actions share gameplay availability and disabled state with the visible controls. The stable movement-action identifiers (`moveUpButton`, `moveDownButton`, `moveLeftButton`, `moveRightButton`) remain available for accessibility testing; the Grapple control is `grappleButton`.
* **Haptics:** engaging or changing a movement direction produces subtle feedback, Grapple aim-direction changes produce selection feedback, and firing Grapple produces impact feedback. Continued held movement does not vibrate repeatedly.
* **Control layout:** the two-thumb control dock places the joystick on the left and Grapple on the right in the **Standard** layout. **Left-Handed** swaps them, placing Grapple on the left and the joystick on the right. The selected control layout is a persistent gameplay setting.
* Health remains readable as text even though the legacy heart is also shown. Important collection, health, chest, mine, and completion events generate one accessibility announcement per event. Reduce Motion replaces travel-heavy feedback with a fade.
...
## Levels 5–8 Java parity and deliberate corrections

Level 5 retains 52 internal wall anchors and 61 lava anchors. Its visible left doorway (rows 8–11, columns 0–3) returns to Level 4's right entry, intentionally correcting Java's contradictory `GetEntryGrid`; its top exit transitions to Level 7. Its chest interacts at row 52, column 4 but renders with `ChestSide` at row 52, column 8. The native footprint-safe player starts are row 9, column 7 for the left entry and row 5, column 29 for the top entry, keeping Lidia's complete collision region out of walls and lava. The Skeleton likewise starts at row 28, column 30 so its complete footprint is clear of the production map's walls, lava, doors, player entries, and Flying Terror. Its emitters remain at rows/columns 55/16 and 26/43.

Level 6 contains 54 internal wall anchors and 49 lava anchors. The bottom doorway (rows 56–59, columns 27–32) returns to Level 4's top entry; the upper-right exit (rows 0–3, columns 51–56) progresses to Level 8's left-side entry. The footprint-safe bottom player entry is row 53, column 29, and moving right one cell is its verified safe movement into the playable interior. Java used `(5,23)` for the top entry, but that position is not aligned with Native Level 6's upper-right top exit/doorway. The native conversion intentionally corrects the top entry to row 5, column 53 (`(5,53)`), so the implemented Level 8 → Level 6 reverse route places Lidia beneath the correct doorway with a valid collision footprint. This intentional Java correction must not be reverted. Chests at 4/24 (`ChestSide`) and 44/8 (`ChestFront`) independently award 100 score and up to two health. Emitters are at 13/21 and 39/53. Java's overlapping enemy-at-exit defect is corrected with deterministic, footprint-safe Skeleton 22/53 and Flying Terror 10/52 starts.
